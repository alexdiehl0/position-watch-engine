...
# Words that match a request to a stock's industry (Finnhub) or sector (Yahoo).
# Deliberately generous: a request for "energy" should reach refiners and pipelines.
SECTOR_WORDS = {
    "energy": ("energy", "oil", "gas", "petroleum", "refin", "pipeline", "drill", "coal"),
    "renewables": ("renewable", "solar", "wind", "clean energy", "hydrogen"),
    "utilities": ("utilit", "electric", "power", "water"),
    "financials": ("bank", "financ", "insur", "capital markets", "asset manage", "credit"),
    "banks": ("bank",),
    "insurance": ("insur",),
    "healthcare": ("health", "pharma", "biotech", "medical", "life sciences"),
    "technology": ("tech", "software", "semiconduct", "hardware", "internet", "it services"),
    "semiconductors": ("semiconduct", "chip"),
    "industrials": ("industrial", "machin", "aerospace", "defen", "transport", "construct", "electrical equipment"),
    "defence": ("defen", "aerospace"),
    "consumer staples": ("staple", "food", "beverage", "household", "tobacco", "consumer defensive"),
    "consumer": ("consumer", "retail", "apparel", "restaur", "leisure", "auto"),
    "materials": ("material", "chemical", "metal", "mining", "steel", "paper"),
    "real estate": ("real estate", "reit", "propert"),
    "telecoms": ("telecom", "communication", "media", "wireless"),
}


def _words_for(value: str) -> tuple:
    """The words a request's value should match on: its own, plus the sector's."""
    value = (value or "").strip().lower()
    for name, words in SECTOR_WORDS.items():
        if value == name or value in words or any(w in value for w in words):
            return tuple({*words, value})
    return (value,) if value else ()


def matches_request(symbol: str, entry: dict, request: dict) -> bool:
    """Does this pool stock answer the request? Tickers are exact; a sector or
    theme is matched against the stock's industry, sector and name."""
    value = (request.get("value") or "").strip()
    if request["kind"] == "tickers" or "," in value:
        return symbol.upper() in {v.strip().upper() for v in value.split(",") if v.strip()}
    screen = entry.get("screen") or {}
    haystack = " ".join(
        str(x).lower() for x in (entry.get("industry"), screen.get("sector"), entry.get("name"), symbol) if x
    )
    return any(word and word in haystack for word in _words_for(value))
...
def pick(pool, universe, exclude, today, requests=()):
    ...
    eligible = [
        ...
        and not any(matches_request(s, e, r) for r in avoid)
    ...
    if focus_request:
        ...
        focus = [s for s in by_score if matches_request(s, pool["stocks"][s], focus_request)][:room]
...
def expand_for(pool, universe, request: dict, today, needed: int = 4) -> list:
    ...
    stocks = pool["stocks"]
    matching = [s for s, e in stocks.items() if matches_request(s, e, request)]
    if len([s for s in matching if (stocks[s].get("screen") or {}).get("passed")]) >= needed:
        return []
    seeds = [s for group, symbols in universe["seeds"].items() if _words_for(request.get("value")) and
             any(w in group.lower() for w in _words_for(request.get("value"))) for s in symbols]  # fmt: skip
    added, notes = [], []
