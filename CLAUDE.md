...
| `analysis/pool/` | Stock pool, split by job: `state` (the file and the weekly refresh), `screening` (one row of figures per stock, once a day, and the score), `sectors` (what counts as belonging to a sector or theme), `pick` (the day's watchlist — focus slots, avoid filter and on-demand peer expansion come from the client's requests). |
